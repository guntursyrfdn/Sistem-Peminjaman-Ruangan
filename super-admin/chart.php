...
if (isset($_GET['filterYear'])) {
  ...
  // var_dump($filterYear);
  // die;
  $queryTotal = "SELECT ruangan, COUNT(*) as total FROM jadwal WHERE YEAR(tanggal) = $filterYear GROUP BY ruangan";
} else {
  $queryTotal = "SELECT ruangan, COUNT(*) as total FROM jadwal GROUP BY ruangan";
}
